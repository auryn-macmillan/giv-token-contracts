...
function csvJSON(csv: any, file: string) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!result[currentAddress]) {
            ...
            var previousAllocation = BigNumber.from(
                result[currentAddress].allocation,
            );
            ...
            if (
                currentReason &&
                !result[currentAddress].reasons.includes(currentReason)
            ) {
    ...
    console.log(
        ...
        file,
        "amount:",
        ethers.utils.formatEther(totalTokensFinal.toString()).toString(),
    );
